...
4. **`get_database_schema`**: Returns the schema of **every** table in one call — each with columns, data types, nullability, primary key, and foreign keys. Deterministic (tables sorted by schema/name, columns by ordinal position) so the output is byte-stable across runs and diffable in version control. Per **Rule 10**, this is a separate tool from `get_table_schema` (whole-DB structure vs. one table), not a flag on it.
