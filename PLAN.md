...
- [ ] **Read-Only Guard:** Inside `PostgresDialect.connect(read_only=True)`, force `SET TRANSACTION READ ONLY;`. Read-only enforcement is the dialect's own responsibility — it must never leak upward.
