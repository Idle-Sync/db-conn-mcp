...
10. **`execute_write_query`**: Runs `UPDATE`, `INSERT`, `DELETE`, or DDL. Also accepts `params` and `timeout_ms` (as above). **Defaults to `dry_run=true`** — a preview, not a commit. Gated server-side, in order:
   ...
   - **What the dry-run does:** executes the statement inside a transaction and **always rolls back**, returning the `rows_affected` it *would* have had. Only the `mode` gate applies to the preview itself (nothing commits, so no yolo/consent needed). The permission it grants is scoped to that exact `(database, sql, params)`, **expires after 10 minutes**, and is **consumed** by the commit it authorizes — so the same statement run twice needs two previews. Documented caveat: a dry-run still executes server-side until rollback (brief locks, sequence advancement, trigger side effects).
