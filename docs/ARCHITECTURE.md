...
4. **`content-security-policy: default-src 'self'`, no CORS, `cache-control: no-store`** on every response, 403s included — the page fetches nothing external, no other origin can read it, and nothing carrying `?token=` reaches the browser's disk cache.
   The hint page is the one response served under a different policy, and it is a *stricter* one: `default-src 'none'; style-src 'unsafe-inline'`. It cannot fetch the real stylesheet — that sits behind the guard that just refused it — so it carries a small `<style>` block instead, and pays for that with a default of `'none'` rather than `'self'`: no script, image, font, frame or XHR from anywhere, this origin included. Its body is a module-level constant with no interpolation and no script of any kind, so `'unsafe-inline'` can only ever apply the CSS written in that constant.
